...
urlfile="https://raw.githubusercontent.com/nayfuun/webpage/master/public/post/penguins.csv"

penguins<-read_csv(url(urlfile))
```
...
- Tidymodels is a meta-package (like the tidyverse!) developped by the RStudio team.
...
- Priority is on easy of use, consistency, and redable code (rather that speed)

--

- "Successor" to the widely used [caret package](), especially Parsnip.

--

- Emphasis is on predictive modeling (supervised learning).

--

- Still in development, new packages are popping up fast.
...
- Step one: Choosing the initial centers, this is usually done randomly, though in some variants centers are deliberately set far away from each other (most prominently k-means++).

--

- Step two: Assign each point in the sample to its nearest center.

--

- Step three: Calculate the mean value of all points assigned to every center, this point is now the new center for that cluster.

--

Step two and three are iterated until some stopping condition is met (typically when the difference between the current and new centers falls below a certain threshold).
---
...
--
- The [parsnip package](https://www.tidymodels.org/find/parsnip/) "provide a tidy, unified interface to models that can be used to try a range of models without getting bogged down in the syntactical minutiae of the underlying packages".
...
We can then look at how our models have performed using the [collect metrics]() function:
...
Let's take a closer look at the performance of the logistic regression model, we can easily obtain a confusion matrix as a tidy tibble by using the [conf_mat_resampled]() function:
...
We might be **overfitting** the data! To check that our model will perform well on new data we have to see how it performs on our testing data.

--

We can do this easily using the [last_fit]() function, this takes the model which performs the best on our training data, and fits it to the test data:
...
As with our training data, we can use [collect_metrics]() and [collect_predictions]() to look at the performance of our model:
...
We can use the [tidy function]() from the broom package to get the coefficients for our model:
